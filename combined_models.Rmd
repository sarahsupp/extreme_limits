...
molt.dat$molt3class <- as.character(molt.dat$PriMary.Molt)
molt.dat$molt3class[molt.dat$molt3class %in% c("M", "R", "1", "2", "3", "4", "5", "6", "7", "8")] <- "early"
molt.dat$molt3class[molt.dat$molt3class %in% c("9", "0")] <- "late"
molt.dat$molt3class[molt.dat$molt3class %in% c("F", "L")] <- "complete"
...
molt.dat$molt3class <- ordermolt3(molt.dat$molt3class)
...
ggplot(sd, aes(molt3class)) + geom_histogram() + xlab("Primary Molt Stage") + ylab("number birds") +
 theme_bw() +  facet_wrap(~yearfac)
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height=4, fig.width=6}
mq50 <- MOLT.quants.3[,c(1:3,6)]
mq50 <- mq50[complete.cases(mq50),]

ggplot(mq50, aes(yr, MOLT.quant.50.3)) + geom_point(aes(size=n.birds), alpha=0.5) + 
  xlab("Year") + ylab("Median molt stage") + theme_bw()

moltscores <- table(mq50$MOLT.quant.50.3)
...
sd <- body.dat[complete.cases(body.dat),]
ggplot(sd, aes(Fat)) + geom_histogram(binwidth=0.25) + xlab("Fat Score") + ylab("number birds") +
 theme_bw() +  facet_wrap(~yearfac)
```
