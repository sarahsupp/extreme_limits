...
#load("C:\\Users\\pbeck.WHRC\\Dropbox\\Hummers\\Hummer_code\\P4_Extreme_events_Broad_tailed\\Results\\annual.anom.df.rdata")
load("A:\\Share\\pbeck\\Hummer_NASA\\Code_copy\\annual.anom.df.rdata")

yearly.climdat <- function(tapply.out,nm){
  tapply.out <-as.data.frame(tapply.out);tapply.out <- cbind.data.frame(tapply.out,as.numeric(rownames(tapply.out)));
  colnames(tapply.out)[2]<-"yr"
  colnames(tapply.out)[1]<- nm  
  return(tapply.out)
}
#debug(yearly.climdat)
...
yearly._1NDVI <- yearly.NDVI; yearly._1NDVI$minNDVI <- -1*yearly._1NDVI$NDVI
...
winterend._1NDVI <- winterend.NDVI; winterend._1NDVI$minwinterNDVI <- -1*winterend._1NDVI$winterNDVI
