...
mod.evaluation <- function(yname,D = "yearly.Te.10C.q",R = "NDVI",N = "n.birds",
  ...
  #++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  if (log.D){cat("DEMAND variable will be log-transformed\n");D<-log(D)+1}
  if (log.R){cat("RESOURCE variable will be log-transformed\n");R<-log(R)+1}
  ...
  CWC <- function(x,LOC){mns.x <- tapply(x,LOC,mean,na.rm=T)
                         ...
                         mns.x <- x - mns.x
                         return(mns.x)}  
  if (centering == 'CGM'){D <- CGM(D) ; R <- CGM(R)
  ...
  win.graph(w=9,h=4);par(mfrow=c(1,3))
  hist(unique(D),col="light grey");hist(unique(R),col="light grey");plot(R,D,pch=16)
  ...
  Akaike.weight <- function(AIC.vec){AIC.vec <- AIC.vec-min(AIC.vec,na.rm=T)
  ...
  get.conf.int <- function(p.est,SE){rep(p.est,each=2)+rep(SE,each=2)*c(-1.96,1.96)}
